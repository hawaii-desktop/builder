...
\n\
[Service]\n\
ExecStart=twistd --nodaemon --no_save -y /data/buildbot/master/buildbot.tac\n\
User=admin\n" > \
    /etc/systemd/system/multi-user.target.wants/buildbot-master.service

# Make host port 8010 match container port 8010
expose :8010

# Make host port 9989 match container port 9989
expose :9989
